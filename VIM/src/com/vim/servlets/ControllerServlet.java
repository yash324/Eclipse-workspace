...
public class ControllerServlet extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		if (VIEW_CAR_LIST_ACTION.equals(actionName)) {
			...
			// Create a new CarDTO and set in request with attribute name as
			// 'car'
			...
			// Find the respective car (CarDTO) from carDAO using appropriate
			// API of DAO
			request.setAttribute("car", car);
			...
			int id = Integer.parseInt(request.getParameter("id"));
			car.setId(id);
			// TODO If it is a new car then invoke create api of DAO else update
			// api
			if (id == -1)
				carDAO.create(car);
			else
		...
		// appropriate destination page set in above if else blocks depending on
		// action.
